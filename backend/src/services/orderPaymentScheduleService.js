...
async function customerPaidAmounts(orders) {
  ...
  for (const order of orders) {
    for (const item of order.ERP_order_items || []) {
      ...
      owner.set(Number(item.id), Number(order.id));
    }
  }
  const links = await ItemGroupItem.findAll({
    where: { orderItemId: [...items.keys()] },
    attributes: ["groupId", "orderItemId"],
  });
  const groups = [...new Set(links.map((link) => Number(link.groupId)).filter(Boolean))];
  if (groups.length) {
    const [allLinks, payments] = await Promise.all([
      ItemGroupItem.findAll({ where: { groupId: groups }, attributes: ["groupId", "orderItemId"] }),
      Payment.findAll({ where: { groupId: groups, status: "completed" }, attributes: ["groupId", "amount"] }),
    ]);
    const extraIds = [...new Set(allLinks.map((link) => Number(link.orderItemId)).filter((id) => id && !items.has(id)))];
    if (extraIds.length) {
      const extra = await OrderItem.findAll({
        where: { id: extraIds },
        attributes: ["id", "quantity", "price", "damagedQty", "giftQty"],
      });
      for (const item of extra) items.set(Number(item.id), item);
    }
    const linesByGroup = new Map();
    for (const link of allLinks) {
      const group = Number(link.groupId);
      if (!linesByGroup.has(group)) linesByGroup.set(group, new Map());
      linesByGroup.get(group).set(Number(link.orderItemId), billable(items.get(Number(link.orderItemId))));
    }
    const paymentByGroup = new Map();
    for (const payment of payments) {
      const group = Number(payment.groupId);
      paymentByGroup.set(group, money2((paymentByGroup.get(group) || 0) + Number(payment.amount || 0)));
    }
    for (const [group, lines] of linesByGroup) {
      const total = money2([...lines.values()].reduce((sum, amount) => sum + amount, 0));
      const amount = paymentByGroup.get(group) || 0;
      if (total <= BILLABLE_EPSILON || amount <= BILLABLE_EPSILON) continue;
      const shares = new Map();
      for (const [itemId, lineTotal] of lines) {
        const orderId = owner.get(itemId);
        if (orderId) shares.set(orderId, money2((shares.get(orderId) || 0) + lineTotal));
      }
      const entries = [...shares.entries()];
      let allocated = 0;
      entries.forEach(([orderId, share], index) => {
        const allocation = index === entries.length - 1
          ? money2(amount - allocated)
          : money2((amount * share) / total);
        allocated = money2(allocated + allocation);
        paid.set(orderId, money2((paid.get(orderId) || 0) + allocation));
      });
    }
  }
  for (const order of orders) {
...
async function runInstallmentReminders(kind, table, targetDate) {
  ...
  const [orders, schedules, userIds] = await Promise.all([
    kind === "customer"
      ? Order.findAll({
          where: { id: orderIds },
          include: [
  ...
  const paid = kind === "customer"
    ? await customerPaidAmounts(orders)
    : new Map((await SupplierOrderPayment.findAll({
  ...
  for (const row of due) {
    ...
    const installment = order && applyFifoPaidToInstallments(
      schedules.get(Number(row.orderId)) || [],
      paid.get(Number(row.orderId)) || 0,
...
export async function runPaymentInstallmentReminders() {
  ...
  const [customer, supplier] = await Promise.all([
    runInstallmentReminders("customer", "ERP_order_payment_installments", targetDate),
    runInstallmentReminders("supplier", "ERP_supplier_order_payment_installments", targetDate),
  ]);
